...
const FileUploadForm = ({ onNext, onPrevious, initialData }) => {
  ...
  const validateFile = (uploadedFile) => {
    // 파일 확장자 검사
    if (!uploadedFile.name.toLowerCase().endsWith('.hwpx')) {
  ...
  const handleFileChange = (e) => {
    const selectedFile = e.target.files[0]; // 첫 번째 파일만 사용
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex flex-col items-center">
              {/* 클라우드 아이콘 */}
              <svg
                className={`w-16 h-16 mb-4 ${error ? 'text-red-500' : 'text-blue-500'}`}
                fill="currentColor"
                viewBox="0 0 24 24"
              ...
              <p className="text-sm text-gray-500 mb-6">HWPX 파일만 지원</p>
